...
#### BlendMode
`AnimationBlendMode` (VBP NodeFunction)
이 트랙이 **레이어 내부에서** 다른 트랙들과 합성되는 방식입니다. (레이어 자체의 BlendMode와는 별개입니다)
*   **`WeightedSet`**: 같은 레이어 내 다른 트랙들과 Weight 비율대로 섞입니다. (BlendSpace 등 구현 시 사용)
*   **`Override`**: 같은 레이어 내 이전 값을 무시하고 덮어씁니다.
*   **`Additive`**: (`Add`) 현재 값 위에 더합니다.
...
#### fromData
(data: AnimationData, target: Model, modelName: string?, params: AnimationTrackParams?) -> (AnimationTrack)
...
*   `modelName`: 데이터 내에서 사용할 모델 이름. (기본값: `"Default"`)
