...
VBP 노드의 우선순위입니다.
*   **0 ~ 9**: 기본 동작 (Idle, Walk)
*   **10 ~ 19**: 행동 (Attack, Interact)
*   **20 ~ 29**: 반응/FX (Hit, Recoil)
*   **100+**: 강제 오버라이드 (Cinematic, Cutscene)

#### BlendMode
`AnimationBlendMode`
이 레이어가 그래프의 다른 노드들과 합성되는 방식입니다. `ValueByPriority`의 **NodeFunction**에 매핑됩니다.

*   **`WeightedSet`** (Default): 같은 Priority 그룹 내에서 `Weight` 비율에 따라 혼합됩니다. (BlendSpace)
*   **`Add`**: 하위 Priority의 결과값 위에 현재 값을 더합니다. (Additive Animation)
*   **`Multiply`**: 하위 Priority의 결과값에 현재 값을 곱합니다.
*   **`Override`**: 하위 값을 무시하고 덮어씁니다. (`Set`과 유사하지만 Weight 적용 가능)
