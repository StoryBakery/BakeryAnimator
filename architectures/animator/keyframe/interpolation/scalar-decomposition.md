...
- **정밀한 제어**
  `Vector3`의 X축은 Linear로, Y축은 Bezier로 움직여야 할 때가 많습니다. 통짜 데이터로는 이를 제어할 수 없습니다.
- **베지어 수학**
  베지어 곡선 공식은 기본적으로 1차원 값에 대해 작동합니다.
- **블렌더 호환성**
  블렌더는 Transform 데이터를
  - `Location X, Y, Z`
...
- `Color3`: `R`, `G`, `B`
- `Color3Uint8`: `R`, `G`, `B` (Color3 와 달리 $[0, 1]$ 범위)
...
- Rotation: [회전 모드](../../transform/rotation/index.md) 에 따라 나눕니다.
