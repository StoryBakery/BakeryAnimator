...
블렌더에서의 `Amature` 모디파이어가 있는 메쉬 오브젝트의 이름과
`Amature` 의 `Bone` 이름 일치할 경우 `Motor6D`로 변환합니다.
일치하지 않을 경우 `Bone` 인스턴스로 변환됩니다.

### 회전 보정

로블록스에서 `glTF` 든 `fbx` 든 `Motor6D` 가 파트와 연결될 때
초기 회전(Rest Rotation)이 (0,0,0)으로 리셋되는 특성이 있습니다.
...
1. **Neutral Pose Correction**
   블렌더 Bone의 월드 회전 축을 로블록스 Motor6D C0/C1 기준에 맞춰 보정.
2. **Transform Calculation**
   `m.Transform = (Part0.CFrame * C0):Inverse() * (Part1.CFrame * m.C1)`
   공식을 역이용하여 정확한 포즈 적용.
