...
블렌더를 단순한 모델링 도구가 아닌, 애니메이션의 저작 도구(Authoring Tool)로 활용하며, 로블록스로의 매끄러운 데이터 이전을 목표로 합니다.
...
### Motor6D Creation Rule
*   Bone 이름과 Mesh Object 이름이 일치(Match)할 경우 `Motor6D`로 변환.
*   일치하지 않을 경우 `Bone` 인스턴스로 변환.

### Rotation Correction
Motor6D는 파트와 연결될 때 초기 회전(Rest Rotation)이 (0,0,0)으로 리셋되는 특성이 있습니다.
